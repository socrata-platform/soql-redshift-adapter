...
trait SoQLValueDebugHelper {
  implicit def hasDocCV(implicit cryptProviderProvider: CryptProviderProvider) =
    new HasDoc[SoQLValue] {
      ...
      private def cryptProviderFor(
          provenance: Option[Provenance]
      ): CryptProvider =
        provenance
          .flatMap(cryptProviderProvider.forProvenance)
          .getOrElse(CryptProvider.zeros)
    }
}
