...
final class DatabaseEntityMetaTypes(
  ...
  val provenanceMapper = new types.ProvenanceMapper[DatabaseEntityMetaTypes] {
    private val dtnMap =
      new scm.HashMap[Provenance, DatabaseTableName[DatabaseTableNameImpl]]
    private val provMap = new scm.HashMap[
      (String, Long /* Dataset.internalName, CopyNumber */ ),
      Provenance
    ]

    def fromProvenance(
        prov: Provenance
    ): types.DatabaseTableName[DatabaseEntityMetaTypes] = {
      dtnMap(prov)
    }

    def toProvenance(
        dtn: types.DatabaseTableName[DatabaseEntityMetaTypes]
    ): Provenance = {
  ...
  def rewriteFrom(
    ...
    analysis.rewriteDatabaseNames[DatabaseEntityMetaTypes](
      { case DatabaseTableName((dsid, stage)) =>
        DatabaseTableName(
          datasetService
            .findByInternalNameAndPublishedState(
              dsid.underlying,
              stage.underlying
            )
            .get
      ...
      {
        case (
              DatabaseTableName((dsid, stage)),
              DatabaseColumnName(colId: UserColumnId)
            ) =>
          DatabaseColumnName {
            ...
            columnService
              .findByDatasetIdAndUserColumnId(
                dataset.systemId,
                colId.underlying
              )
              .get
