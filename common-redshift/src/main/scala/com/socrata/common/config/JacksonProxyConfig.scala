...
object JacksonProxyConfigBuilder {
  ...
  def merge(mainNode: JsonNode, updateNode: JsonNode): JsonNode = {
    ...
    while (fieldNames.hasNext) {
      ...
      if (jsonNode != null && jsonNode.isObject) merge(jsonNode, updateNode.get(fieldName)) else mainNode match {
        case node: ObjectNode => val value = updateNode.get(fieldName)
...
case class JacksonProxyConfigBuilder(private val objectMapper: ObjectMapper) extends ConfigBuilder {
  override def withSources(configSources: ConfigSource*): JacksonProxyConfigProvider = JacksonProxyConfigProvider( // Merges config sources into a JsonNode, starting from a blank empty JsonNode
    merge(configSources, objectMapper.createObjectNode().asInstanceOf[JsonNode]), objectMapper, new Properties())

  override def withEnvs(configEnvs: EnvSource*): JacksonProxyConfigProvider = JacksonProxyConfigProvider(objectMapper.createObjectNode().asInstanceOf[JsonNode], objectMapper, merge(configEnvs, new Properties()))
}
...
case class JsonNodeBackedJacksonInvocationHandler(data: JsonNode, objectMapper: ObjectMapper, env: Properties) extends InvocationHandler {
  def invoke(proxy: scala.AnyRef, method: Method, args: Array[AnyRef]): AnyRef = {
    val out = method.getName match { // Simple implementation for toString, when the root interface is accessed directly.
      case "toString" => data.toString // implement others when it becomes important...ie equals/hash...not sure this matters much for our configs yet?
...
case class JacksonProxyConfigProvider(private val data: JsonNode, private val objectMapper: ObjectMapper, private val env: Properties) extends ConfigProvider with ConfigBuilder {
  ...
  override def withSources(configSources: ConfigSource*): JacksonProxyConfigProvider = JacksonProxyConfigProvider( // Merge new sources with existing JsonNode
    merge(configSources, data), objectMapper, env)

  override def withEnvs(configEnvs: EnvSource*): ConfigBuilder with ConfigProvider = JacksonProxyConfigProvider( // Merge new sources with existing JsonNode
    data, objectMapper, merge(configEnvs, env))

  def chunk(path: String): JacksonProxyConfigProvider = JacksonProxyConfigProvider(data.findValue(path), objectMapper, env)

  def proxy[T](path: String, clazz: Class[T]): T = { // First extract the value at the json path from the root node, then use this as the proxy data
  ...
  private def proxy[T](clazz: Class[T], newData: JsonNode): T = { // We will return a proxy that will delegate method calls that are backed by a JsonNode and Jackson
    Proxy.newProxyInstance( // Use the classLoader of the target class?
      ...
      JsonNodeBackedJacksonInvocationHandler(newData, objectMapper, env) // Our proxy will implement our target interface/trait (clazz), so we will force cast it to look like this type
    ).asInstanceOf[T]
  }
